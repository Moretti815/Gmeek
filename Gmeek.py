...
class GMEEK():
    ...
    def createPlistHtml(self):
        ...
        # Add friends page to singeListJson
        if "friends" in self.blogBase["singlePage"]:
            import time
        ...
        # Add addlink page to singeListJson
        if "addlink" in self.blogBase["singlePage"]:
            import time
            self.blogBase["singeListJson"]["addlink"]={
                "postTitle": self.i18n["addlink"],
                "labels": ["addlink"],
        ...
        # Add safego page to singeListJson
        if self.blogBase.get("safego") and self.blogBase["safego"].get("enable"):
            import time
            self.blogBase["singeListJson"]["go"]={
                "postTitle": self.i18n.get("safego", "Safe Go"),
                "labels": ["go"],
                "postUrl": "go.html",
                "description": self.i18n.get("safegoSubtitle", "External Link Safety Check"),
                "createdAt": int(time.time())
            }
    ...
    def createFriendsHtml(self):
        ...
        friendsBase["description"]=friendsBase.get("friendsDesc", "亦师亦友 · 给予帮助的人最可爱！")
    ...
    def createAddlinkHtml(self):
        addlinkBase=self.blogBase.copy()
        addlinkBase["postTitle"]=self.i18n["addlink"]
        addlinkBase["description"]=addlinkBase.get("addlinkDesc", "期待与您建立友好的互联网连接！")
        addlinkBase["postUrl"]=self.blogBase["homeUrl"]+"/addlink.html"
        addlinkBase["ogImage"]=self.blogBase["ogImage"]
        addlinkBase["commentNum"]=0
        addlinkBase["style"]=""
        addlinkBase["script"]=""
        addlinkBase["head"]=""
        addlinkBase["top"]=False
        addlinkBase["postSourceUrl"]=""
        addlinkBase["repoName"]=self.options.repo_name
        addlinkBase["bottomText"]=""
        addlinkBase["needComment"]=0
        addlinkBase["highlight"]=0

        keys=['sun','moon','sync','home']
        addlinkIcon=dict(zip(keys, map(IconBase.get, keys)))
    ...
    def createGoHtml(self):
        goBase=self.blogBase.copy()
        goBase["postTitle"]=self.i18n.get("safego", "Safe Go")
        goBase["description"]=goBase.get("safegoDesc", "External Link Safety Check")
        goBase["postUrl"]=self.blogBase["homeUrl"]+"/go.html"
        goBase["ogImage"]=self.blogBase["ogImage"]
        goBase["commentNum"]=0
        goBase["style"]=""
        goBase["script"]=""
        goBase["head"]=""
        goBase["top"]=False
        goBase["postSourceUrl"]=""
        goBase["repoName"]=self.options.repo_name
        goBase["bottomText"]=""
        goBase["needComment"]=0
        goBase["highlight"]=0

        keys=['sun','moon','sync','home']
        goIcon=dict(zip(keys, map(IconBase.get, keys)))
